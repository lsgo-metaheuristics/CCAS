...
namespace CEC2010 {
	...
	Benchmarks::Benchmarks(){
		...
		for (int j = 0; j < dimension; ++j)
			group.push_back(j);
		idealGroups.push_back(group);

		Pvector = NULL;		
	}


	void Benchmarks::createIdealGroups()
	{
		vector<unsigned> s;
    unsigned s_size;
		...
		compute(&x[0]); //create the true PVector :-)

		this->idealGroups.clear();
		...
		{
			s_size = 1;
			s.push_back(nonSeparableGroupSize);
		}
		else if (this->ID <= 13)
		{
			s_size = dimension / (2 * nonSeparableGroupSize);
      for (unsigned i = 0; i < s_size; ++i)
				s.push_back(nonSeparableGroupSize);
		}
		else if (this->ID <= 18)
		{
			s_size = dimension / (nonSeparableGroupSize);
      for (unsigned i = 0; i < s_size; ++i)
				s.push_back(nonSeparableGroupSize);
		}
		else if (this->ID <= 20)
		...
		//non-separable groups
		for (unsigned i = 0; i < s_size; ++i)
		{
			vector<unsigned> group;

      for (unsigned j = c; j < c + s[i]; ++j)
				group.push_back(Pvector[j]);

			this->idealGroups.push_back(group);
		...
		/*
		//separable groups  		
		int sizeOfSeparableSubcomponents = 10;
		for (int j = c; j < dimension; j += sizeOfSeparableSubcomponents)
		{
			vector<unsigned> group;
			for (int k = 0; k < sizeOfSeparableSubcomponents; ++k)
				group.push_back(Pvector[j + k]);
			this->idealGroups.push_back(group);
		}
		*/

		//separable groups (if any)		
		vector<unsigned> group;
